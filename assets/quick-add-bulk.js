if (!customElements.get('quick-add-bulk')) {
  customElements.define(
    ...
    class QuickAddBulk extends BulkAdd {
      ...
      handleBulkError(message) {
        const existingError = document.querySelector('.bulk-error-notification');
        if (existingError) {
          existingError.remove();
        }
        
        const errorDiv = document.createElement('div');
        errorDiv.className = 'bulk-error-notification';
      ...
      onCartUpdate() {
        return new Promise((resolve, reject) => {
          fetch(`${this.getSectionsUrl()}?section_id=${this.closest('.collection').dataset.id}`)
            ...
            .catch((e) => {
              this.handleBulkError('Unable to update items. Please refresh and try again.');
              reject(e);
            });
        });
      }
