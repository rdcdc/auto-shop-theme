...
if (!customElements.get('quick-order-list')) {
  customElements.define(
    ...
    class QuickOrderList extends BulkAdd {
      ...
      handleQuickOrderError(message) {
        const existingError = document.querySelector('.quick-order-error-notification');
        if (existingError) {
          existingError.remove();
        }
        
        const errorDiv = document.createElement('div');
        errorDiv.className = 'quick-order-error-notification';
      ...
      refresh() {
        return new Promise((resolve, reject) => {
          fetch(`${this.getSectionsUrl()}?section_id=${this.sectionId}`)
            ...
            .catch((e) => {
              this.handleQuickOrderError('Unable to refresh order list. Please try again.');
              reject(e);
            });
        });
      }
