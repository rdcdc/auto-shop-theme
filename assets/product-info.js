  ...
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      handleProductError(message) {
        const existingError = document.querySelector('.product-error-notification');
        if (existingError) {
          existingError.remove();
        }
        
        const errorDiv = document.createElement('div');
        errorDiv.className = 'product-error-notification';
        ...
        fetch(requestUrl, { signal: this.abortController.signal })
          ...
          .catch((error) => {
            if (error.name === 'AbortError') {
              ...
              this.handleProductError('Unable to load product information.');
            }
          });
      }
      ...
      updateURL(url, variantId) {
        this.querySelector('share-button')?.updateUrl(
          `${window.shopUrl}${url}${variantId ? `` : ''}`
        );

        if (this.dataset.updateUrl === 'false') return;
        window.history.replaceState({}, '', `${url}${variantId ? `` : ''}`);
      }
      ...
      fetchQuantityRules() {
        ...
        fetch(`${this.dataset.url}?variant=${currentVariantId}&section_id=${this.dataset.section}`)
          ...
          .catch((e) => this.handleProductError('Unable to update quantity rules.'))
